...
- [x] 使用 `*InsertInput` 统一 insert/insert_many/upsert 接受的 Insert、模型、InsertDict 和完整模型 Dict, 序列化时只提取数据库列并忽略关系字段
- [x] `asdict()` 支持 `keep`、`fetch`、`empty`、`omit` 四种关系策略; 生成 `*ScalarDict` 表示省略关系后的标量快照, 并可直接作为 `*InsertInput`
- [x] sqlite 后端:
...
- [x] SQLite 模型通过 `without_rowid()` 方法或 property 声明 `WITHOUT ROWID` 表, 返回 `False` 时禁用, 并在 schema 选项变化时确认重建

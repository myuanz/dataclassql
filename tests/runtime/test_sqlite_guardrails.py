...
def test_unknown_write_columns_are_ignored(tmp_path: Path) -> None:
    db_path = tmp_path / "runtime.db"
    prepare_database(db_path)
    _, client = build_client()
    user_table = client.runtime_user

    inserted_one = user_table.insert({"name": "Insert", "email": None, "typo": 1})
    assert inserted_one.name == "Insert"

    inserted_many = user_table.insert_many([
        {"name": "InsertMany", "email": None, "typo": 1},
    ])
    assert [item.name for item in inserted_many] == ["InsertMany"]
    ...
    assert fetched.name == "Upsert"
    assert fetched.email == "update-many@example.com"
    client.__class__.close_all()
